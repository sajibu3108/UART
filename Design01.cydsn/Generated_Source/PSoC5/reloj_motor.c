...
/*******************************************************************************
* Function Name: reloj_motor_Start
********************************************************************************
*
* Summary:
...
void reloj_motor_Start(void) 
{
    ...
    reloj_motor_CLKEN |= reloj_motor_CLKEN_MASK;
	reloj_motor_CLKSTBY |= reloj_motor_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: reloj_motor_Stop
********************************************************************************
*
* Summary:
...
void reloj_motor_Stop(void) 
{
    ...
    reloj_motor_CLKEN &= (uint8)(~reloj_motor_CLKEN_MASK);
	reloj_motor_CLKSTBY &= (uint8)(~reloj_motor_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: reloj_motor_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((reloj_motor_CLKEN & reloj_motor_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = reloj_motor_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = reloj_motor_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* reloj_motor__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(reloj_motor_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(reloj_motor_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: reloj_motor_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: reloj_motor_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = reloj_motor_GetSourceRegister();
    uint16 oldDivider = reloj_motor_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = reloj_motor_CLKEN & reloj_motor_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((reloj_motor_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: reloj_motor_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(reloj_motor_DIV_PTR);
}


/*******************************************************************************
* Function Name: reloj_motor_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    reloj_motor_MOD_SRC |= modeBitMask & (uint8)reloj_motor_MODE_MASK;
}


/*******************************************************************************
* Function Name: reloj_motor_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    reloj_motor_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(reloj_motor_MODE_MASK));
}


/*******************************************************************************
* Function Name: reloj_motor_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return reloj_motor_MOD_SRC & (uint8)(reloj_motor_MODE_MASK);
}


/*******************************************************************************
* Function Name: reloj_motor_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = reloj_motor_GetDividerRegister();
    uint8 oldSrc = reloj_motor_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        reloj_motor_MOD_SRC |= CYCLK_SSS;
        reloj_motor_MOD_SRC =
            (reloj_motor_MOD_SRC & (uint8)(~reloj_motor_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: reloj_motor_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return reloj_motor_MOD_SRC & reloj_motor_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: reloj_motor_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    reloj_motor_PHASE = clkPhase & reloj_motor_PHASE_MASK;
}


/*******************************************************************************
* Function Name: reloj_motor_GetPhase
********************************************************************************
*
* Summary:
...
{
    return reloj_motor_PHASE & reloj_motor_PHASE_MASK;
}
