...
void reloj3_Start(void) 
{
    ...
    reloj3_CLKEN |= reloj3_CLKEN_MASK;
	reloj3_CLKSTBY |= reloj3_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: reloj3_Stop
********************************************************************************
*
* Summary:
...
void reloj3_Stop(void) 
{
    ...
    reloj3_CLKEN &= (uint8)(~reloj3_CLKEN_MASK);
	reloj3_CLKSTBY &= (uint8)(~reloj3_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: reloj3_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((reloj3_CLKEN & reloj3_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = reloj3_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = reloj3_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(reloj3_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(reloj3_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: reloj3_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: reloj3_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = reloj3_GetSourceRegister();
    uint16 oldDivider = reloj3_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = reloj3_CLKEN & reloj3_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((reloj3_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: reloj3_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(reloj3_DIV_PTR);
}


/*******************************************************************************
* Function Name: reloj3_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    reloj3_MOD_SRC |= modeBitMask & (uint8)reloj3_MODE_MASK;
}


/*******************************************************************************
* Function Name: reloj3_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    reloj3_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(reloj3_MODE_MASK));
}


/*******************************************************************************
* Function Name: reloj3_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return reloj3_MOD_SRC & (uint8)(reloj3_MODE_MASK);
}


/*******************************************************************************
* Function Name: reloj3_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = reloj3_GetDividerRegister();
    uint8 oldSrc = reloj3_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        reloj3_MOD_SRC |= CYCLK_SSS;
        reloj3_MOD_SRC =
            (reloj3_MOD_SRC & (uint8)(~reloj3_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: reloj3_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return reloj3_MOD_SRC & reloj3_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: reloj3_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    reloj3_PHASE = clkPhase & reloj3_PHASE_MASK;
}


/*******************************************************************************
* Function Name: reloj3_GetPhase
********************************************************************************
*
* Summary:
...
{
    return reloj3_PHASE & reloj3_PHASE_MASK;
}
