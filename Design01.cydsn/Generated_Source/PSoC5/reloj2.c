...
void reloj2_Start(void) 
{
    ...
    reloj2_CLKEN |= reloj2_CLKEN_MASK;
	reloj2_CLKSTBY |= reloj2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: reloj2_Stop
********************************************************************************
*
* Summary:
...
void reloj2_Stop(void) 
{
    ...
    reloj2_CLKEN &= (uint8)(~reloj2_CLKEN_MASK);
	reloj2_CLKSTBY &= (uint8)(~reloj2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: reloj2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((reloj2_CLKEN & reloj2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = reloj2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = reloj2_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(reloj2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(reloj2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: reloj2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: reloj2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = reloj2_GetSourceRegister();
    uint16 oldDivider = reloj2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = reloj2_CLKEN & reloj2_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((reloj2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: reloj2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(reloj2_DIV_PTR);
}


/*******************************************************************************
* Function Name: reloj2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    reloj2_MOD_SRC |= modeBitMask & (uint8)reloj2_MODE_MASK;
}


/*******************************************************************************
* Function Name: reloj2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    reloj2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(reloj2_MODE_MASK));
}


/*******************************************************************************
* Function Name: reloj2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return reloj2_MOD_SRC & (uint8)(reloj2_MODE_MASK);
}


/*******************************************************************************
* Function Name: reloj2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = reloj2_GetDividerRegister();
    uint8 oldSrc = reloj2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        reloj2_MOD_SRC |= CYCLK_SSS;
        reloj2_MOD_SRC =
            (reloj2_MOD_SRC & (uint8)(~reloj2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: reloj2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return reloj2_MOD_SRC & reloj2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: reloj2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    reloj2_PHASE = clkPhase & reloj2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: reloj2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return reloj2_PHASE & reloj2_PHASE_MASK;
}
