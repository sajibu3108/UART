...
char displayStrang[16] = {'\0'};
char displayStrdist[16] = {'\0'};
...
{    
    ...
    UART_Start();
    sprintf(displayStr,"%.2f r %.1f cm",volts, distancia);
    LCD_Position(1,0);
    ...
    {  
       sprintf(displayStr,"%.2f r %.1f cm",volts, distancia);
       sprintf(displayStrang,"%.2f",angulo);
       sprintf(displayStrdist,"%.1f",distancia);
       LCD_Position(1,0);
       LCD_PrintString(displayStr);
    
       UART_PutString(displayStrang);
       UART_PutString("\n\r");
       UART_PutString(displayStrdist);
       UART_PutString("\n\r");
    } // for
